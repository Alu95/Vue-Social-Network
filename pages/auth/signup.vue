        ...
        <md-card md-with-hover>
          ...
          <md-card-content>
            ...
            <md-field :class="{'md-invalid':!validateState('name')}">
              ...
              ></md-input>
              <span class="md-error" id="name-fb">
                <p
                  v-for="(frontErr,index) in veeErrors.collect('email')"
                  :key="index"
                >{{ frontErr }}</p>
              </span>
            </md-field>
            ...
            <md-field :class="{'md-invalid':!validateState('email')}">
              ...
              ></md-input>
              <span class="md-error" id="email-fb">
                <p
                  v-for="(frontErr,index) in veeErrors.collect('email')"
                  :key="index"
                >{{ frontErr }}</p>
            ...
            <md-field :class="{'md-invalid':!validateState('password')}">
              ...
              ></md-input>
              <span class="md-error" id="password-fb">
                <p
                  v-for="(frontErr,index) in veeErrors.collect('password')"
                  :key="index"
                >{{ frontErr }}</p>
              </span>
            </md-field>
          ...
          <md-card-actions>
            <md-button @click="resetForm">
              <md-ripple>Cancel</md-ripple>
            </md-button>
            <md-button @click="onSubmit" :disabled="disabledBtn">
              <md-ripple>SignUp</md-ripple>
            </md-button>
          </md-card-actions>
        </md-card>
      </div>
    ...
    async onSubmit() {
      ...
      await this.$axios
        ...
        .then(res => {
          ...
          this.$auth.loginWith("local", { data: this.form }).catch(err => {
            console.error(err);
